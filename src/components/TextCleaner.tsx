...
export default function TextCleaner() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto md:h-[calc(100vh-2.5rem)] md:min-h-[650px] flex flex-col p-4 md:p-5 space-y-4 overflow-hidden h-auto">
      {/* 头部标题区域 */}
      <header className="flex-shrink-0 flex flex-col md:flex-row md:items-center md:justify-between border-b pb-4 border-gray-200 gap-4">
        <div>
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-4 min-h-0">
        {/* 左侧：输入区 / Diff区域 */}
        <div className="flex-1 h-[300px] sm:h-[360px] md:h-full flex flex-col min-h-0 bg-gray-55 rounded-xl overflow-hidden border border-gray-200 focus-within:border-indigo-400 focus-within:ring-1 focus-within:ring-indigo-400 transition-all shadow-xs">
          <div className="p-3 bg-gray-100/60 border-b border-gray-200 flex justify-between items-center text-sm font-medium text-gray-655 shrink-0 select-none">
          ...
          <div className="p-2.5 border-t border-gray-200 bg-gray-100/50 flex justify-end items-center gap-2 shrink-0">
            {isDiffMode ? (
              <>
                <span className="text-[11px] text-gray-400 mr-auto pl-1 hidden sm:inline">
                  已进入对照试读，右侧可拷贝结果
                </span>
                <button
                  onClick={() => copyToClipboard(processedText, 'cleaned_res')}
                  disabled={!processedText}
                  className="text-xs flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-indigo-50 text-indigo-750 hover:bg-indigo-100 disabled:opacity-50 transition-colors font-medium border border-indigo-100 cursor-pointer"
                  title="直接复制清洗后的最终结果"
                >
              ...
              <button
                ...
                className="text-xs flex items-center gap-1.5 px-3 py-1.5 rounded-md text-gray-655 hover:text-indigo-600 hover:bg-indigo-50 disabled:opacity-50 transition-colors font-medium cursor-pointer"
                title="复制原输入文本"
              >
                {copiedArea === 'original' ? (
        ...
        <div className="hidden md:flex items-center justify-center shrink-0">
          <button
            onClick={handleClean}
            disabled={status === 'cleaning' || !originalText.trim()}
            className="group relative h-10 w-10 flex items-center justify-center rounded-full bg-indigo-650 text-white hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-md transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 cursor-pointer"
            title="开始清洗"
          >
            {status === 'cleaning' ? (
              <Loader2 className="w-4 h-4 animate-spin" />
        ...
        {/* 右侧：输出区 */}
        <div className="flex-1 h-[300px] sm:h-[360px] md:h-full flex flex-col min-h-0 bg-white rounded-xl overflow-hidden border border-gray-200 shadow-xs">
          <div className="p-3 bg-gray-50 border-b border-gray-200 flex justify-between items-center text-sm font-medium text-gray-655 shrink-0 select-none">
            <div className="flex items-center gap-3">
          ...
          <div className="p-2.5 border-t border-gray-200 bg-gray-55 flex justify-end items-center shrink-0">
            {isDiffMode ? (
              <button
              ...
              <button
                ...
                disabled={!processedText}
                className="text-xs flex items-center gap-1.5 px-3.5 py-2 rounded-md text-indigo-750 bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 disabled:opacity-50 transition-all font-semibold cursor-pointer shadow-3xs"
                title="复制清洗格式化后的结果"
              >
